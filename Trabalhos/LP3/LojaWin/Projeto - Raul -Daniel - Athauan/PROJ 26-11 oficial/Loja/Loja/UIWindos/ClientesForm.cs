...
namespace Loja.UIWindows
{
    public partial class ClientesForm : Form
    {
        ...
        private void btnExcluir_Click(object sender, EventArgs e)
        {
            if (codigoTextBox.Text.Length == 0)
            {
                MessageBox.Show("Insira um código de cliente para excluir.");
            }
            else
                try
                {
                    ...
                    ClientesBLL obj = new ClientesBLL();
                    obj.Excluir(int.Parse(codigoTextBox.Text));
        ...
        {
            //pega linha na qual foi clicado
            String posY = Convert.ToString(clientesDataGridView.CurrentCellAddress.Y);
            ClientesBLL obj1 = new ClientesBLL();
            //Percorrer
            MessageBox.Show("O índice do cliente selecionado é " + posY);
        }
